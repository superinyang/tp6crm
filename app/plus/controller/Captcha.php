...
use app\plus\controller\Gif as GIFEncoder;
...
class Captcha
{
    ...
    // 字体路径
    public $font_path = __DIR__.'/zhankukuhei.ttf';
    ...
    private function setRandomCode($im)
    {
        ...
        for ($i = 0; $i < $cou; $i++) {
            ...
            if (preg_match("/[\x7f-\xff]/", $this->random)) {
                ...
                $fontsize = random_int(intval($size), intval($size) + 3);
                $originalcode = 'zxcvbnmasdfghjklqwertyuiop1234567890'; //随机字符串
                $countdistrub = mb_strlen($originalcode);
                $dscode = $originalcode[rand(0, $countdistrub - 1)];
            }
            $color = imagecolorallocate($im, rand(40, 140), rand(40, 140), rand(40, 140));
            imagettftext($im, $fontsize, $jiaodu, intval($x), intval($y), 1, $fonturl, $dscode);
        }
    }
    ...
    private function setCode($im, $string)
    {
        ...
        for ($i = 0; $i < $counts; $i++) {
            ...
            imagettftext($im, $fontsize, $jiaodu, intval($x), intval($y), $color, $fonturl, $char);
        }
    }
